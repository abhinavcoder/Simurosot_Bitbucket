...
inline void linearTransform_new(int &x, int &y, float &newangle,bool invert_field, int teamCode)
{
  int tempx, tempy;
  ...
  x = tempx;
  y = tempy;
  //teamCode is 1 for yellow and 0 for blue
  if(invert_field && teamCode==1)
  newangle = invert_field?normalizeAngle(newangle+PI):newangle;
  else
  newangle = !invert_field?normalizeAngle(newangle+PI):newangle;
	  //y=tempy-800;
...
//          printf("%f %f %f %f\n", homePose[1].x, homePosK[1].x, homePosSigmaSqK[1].x, homeVelocity[1].x);
      ...
      for (int i = 0; i < yellowNum; ++i)
      {
		...
		linearTransform_new(newx, newy, newangle, invert_field , 1);
    ...
    if (ballsNum > 0) {
      ...
      linearTransform_new(newx, newy, garbage,invert_field,0);
